...
<head>
  ...
  <title>SITUGAS – Tambah Siswa</title>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      --blue-mid:#2451d1;--purple-mid:#6d28d9;--purple-pale:#f5f3ff;
      --gray-50:#f8fafc;--gray-100:#f1f5f9;--gray-200:#e2e8f0;
      --gray-400:#94a3b8;--gray-600:#475569;--gray-800:#1e293b;
      --white:#fff;--red:#ef4444;--green:#22c55e;
    ...
    .admin-chip{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);border-radius:999px;padding:3px 12px;font-size:10px;font-weight:700;color:rgba(255,255,255,0.9);letter-spacing:.08em;text-transform:uppercase;display:block;text-align:center;margin-bottom:22px}
    ...
    .form-group input,.form-group select{width:100%;padding:11px 14px;border:1.5px solid var(--gray-200);border-radius:10px;font-size:14px;font-family:inherit;color:var(--gray-800);background:var(--gray-50);outline:none;transition:border-color .2s,box-shadow .2s;appearance:none}
    .form-group input:focus,.form-group select:focus{border-color:var(--purple-mid);background:#fff;box-shadow:0 0 0 3px rgba(109,40,217,.1)}
    .input-with-icon{position:relative}
    .input-with-icon input{padding-right:42px}
    .input-eye-btn{position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--gray-400);padding:4px;display:flex;align-items:center;justify-content:center}
    .input-eye-btn svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2}
    .input-eye-btn:hover{color:var(--gray-800)}
    .form-hint{font-size:11.5px;color:var(--gray-400);margin-top:2px}
    .error-text{font-size:12px;color:var(--red);margin-top:2px;font-weight:600}
    ...
    .btn-submit{padding:11px 28px;background:linear-gradient(135deg,var(--purple-mid),var(--blue-mid));color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;gap:8px;transition:opacity .2s,transform .1s}
    .btn-submit:hover{opacity:.9}
    .btn-submit:active{transform:translateY(1px)}
    .btn-submit svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2.5}
    .btn-cancel{padding:11px 20px;background:var(--white);color:var(--gray-600);border:1.5px solid var(--gray-200);border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;font-family:inherit;transition:border-color .2s}
    .btn-cancel:hover{border-color:var(--gray-400)}
    .alert-error{background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:14px 18px;color:#dc2626;font-size:13px;font-weight:600;margin:0 32px 20px;border-radius:10px}
    .alert-error ul{margin-left:16px;margin-top:4px}
    .alert-error li{margin-top:3px;font-weight:500}

    /* kelas suggestions */
    .kelas-suggestions{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}
    .kelas-chip{padding:4px 12px;background:var(--purple-pale);color:var(--purple-mid);border:1.5px solid #ddd6fe;border-radius:999px;font-size:12px;font-weight:600;cursor:pointer;transition:all .15s}
    .kelas-chip:hover{background:#ede9fe;border-color:var(--purple-mid)}
...
<aside class="sidebar">
  ...
  <nav class="nav-menu">
    ...
    <a href="{{ route('superadmin.kelola-siswa') }}" class="nav-item active">
...
<main>
  ...
  <div class="form-card">
    ...
    <div class="form-card-header">
      ...
      <div>
        <h2 class="form-header-title">Tambah Siswa Baru</h2>
        <p class="form-header-sub">Isi data lengkap siswa yang akan didaftarkan ke sistem</p>
      </div>
    </div>
    ...
    <form method="POST" action="{{ route('superadmin.store-siswa') }}">
      @csrf
      ...
      <div class="form-body">
        ...
        <div class="form-grid">
          ...
          <div class="form-group span-2">
            ...
            <input type="text" name="name" value="{{ old('name') }}"
                   placeholder="Contoh: Andi Pratama" required autocomplete="off">
          ...
          <div class="form-group">
            ...
            <p class="form-hint">NIS digunakan siswa untuk login ke sistem</p>
            @error('nis')<p class="error-text">{{ $message }}</p>@enderror
          </div>

          <div class="form-group">
            <label>Kelas</label>
            <input type="text" name="kelas" id="kelas-input" value="{{ old('kelas') }}"
                   placeholder="Contoh: X-A" required autocomplete="off">
            <p class="form-hint">Gunakan format: X-A, XI-B, XII-IPA-1, dll.</p>
            @error('kelas')<p class="error-text">{{ $message }}</p>@enderror
            <!-- Quick-pick chips dari kelas yang sudah ada -->
            @php
              $kelasAda = \App\Models\User::where('role','siswa')->distinct()->pluck('kelas')->filter()->sort()->values();
            @endphp
            @if($kelasAda->count())
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <div class="input-with-icon">
              <input type="password" name="password_confirmation" id="confirm-field"
                     placeholder="Ulangi kata sandi">
              <button type="button" class="input-eye-btn" id="toggle-confirm">
                <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              </button>
            </div>
            <p class="form-hint" id="pw-match-hint" style="display:none"></p>
          </div>
...
<script>
  ...
  document.getElementById('toggle-confirm').addEventListener('click', () => {
    const f = document.getElementById('confirm-field');
    f.type = f.type === 'password' ? 'text' : 'password';
  });
  ...
  function checkMatch() {
    if (!cf.value) { hint.style.display = 'none'; return; }
