...
<head>
  ...
  <title>SITUGAS – Tambah Guru</title>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{--blue-mid:#2451d1;--purple-mid:#6d28d9;--purple-pale:#f5f3ff;--gray-50:#f8fafc;--gray-100:#f1f5f9;--gray-200:#e2e8f0;--gray-400:#94a3b8;--gray-600:#475569;--gray-800:#1e293b;--white:#fff;--red:#ef4444;--sidebar-w:230px;--radius:14px;--shadow-sm:0 1px 4px rgba(0,0,0,.06);--shadow-md:0 4px 16px rgba(0,0,0,.08)}
...
<aside class="sidebar">
  ...
  <div class="sidebar-footer">
    <div class="user-profile">
      ...
      <div><p class="user-name">{{ auth()->user()->name }}</p><p class="user-role">Super Admin</p></div>
    </div>
...
<main>
  ...
  <div class="form-card">
    <h2 class="form-title">Tambah Guru Baru</h2>
    <p class="form-subtitle">Isi data guru yang akan ditambahkan ke sistem</p>

    @if($errors->any())
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 16px;color:#dc2626;font-size:13px;font-weight:600;margin-bottom:20px;">
        <ul style="margin-left:16px">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul>
      </div>
    @endif

    <form method="POST" action="{{ route('superadmin.store-guru') }}">
      @csrf
      ...
      <div class="form-group">
        <label>NIP (ID Login Guru)</label>
        <input type="text" name="nip" value="{{ old('nip') }}" placeholder="Contoh: 198501012010011001" required>
        <p class="form-hint">NIP digunakan guru untuk login ke sistem</p>
