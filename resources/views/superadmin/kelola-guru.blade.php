...
<head>
  ...
  <title>SITUGAS – Kelola Guru</title>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --blue-mid:#2451d1; --blue-pale:#eef2ff;
      --purple-dark:#4c1d95; --purple-mid:#6d28d9; --purple-pale:#f5f3ff;
    ...
    .badge-purple { background:var(--purple-pale); color:var(--purple-mid); padding:3px 10px; border-radius:999px; font-size:11px; font-weight:700; }
    ...
    .pagination { display:flex; justify-content:flex-end; padding:16px 24px; gap:6px; }
    .page-link { padding:6px 12px; border:1.5px solid var(--gray-200); border-radius:6px; font-size:12px; text-decoration:none; color:var(--gray-600); }
    .page-link.active { background:var(--purple-mid); color:#fff; border-color:var(--purple-mid); }
...
<aside class="sidebar">
  ...
  <nav class="nav-menu">
    ...
    <a href="{{ route('superadmin.kelola-guru') }}" class="nav-item active">
...
<main>
  <div class="topbar">
    <div>
      ...
      <p class="page-subtitle">Tambah, edit, dan hapus data guru</p>
    </div>
    <a href="{{ route('superadmin.tambah-guru') }}" class="btn-primary">
  ...
  <div class="card">
    <div class="card-header">
      <h3 class="card-title">Daftar Guru <span class="badge-purple">{{ $guru->total() }}</span></h3>
    ...
    <table>
      <thead>
        <tr><th>#</th><th>Nama</th><th>NIP</th><th>Aksi</th></tr>
      </thead>
      <tbody>
        @forelse($guru as $i => $g)
        <tr>
          <td style="color:var(--gray-400);font-size:12px;">{{ $guru->firstItem() + $i }}</td>
          <td><strong>{{ $g->name }}</strong></td>
          <td style="font-family:'DM Mono',monospace;font-size:12px;color:var(--gray-400);">{{ $g->nip }}</td>
          <td>
            <div class="action-btns">
              <a href="{{ route('superadmin.edit-guru', $g) }}" class="btn-edit">Edit</a>
              <form method="POST" action="{{ route('superadmin.destroy-guru', $g) }}" onsubmit="return confirm('Hapus guru ini?')">
                @csrf @method('DELETE')
        ...
        @empty
        <tr><td colspan="4" style="text-align:center;padding:30px;color:var(--gray-400);">Belum ada data guru</td></tr>
        @endforelse
      </tbody>
    </table>
    <div class="pagination">{{ $guru->withQueryString()->links() }}</div>
  </div>
</main>
</body>
</html>
